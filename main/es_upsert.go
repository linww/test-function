...
func produceObjectDoc(wg *sync.WaitGroup, docChan chan []elastic.BulkableRequest) {
	...
	for {
		...
		var tSn, mM1, mM2 string
		if standardKeyLength {
			tSn = fmt.Sprintf("sn_%s", RandStringBytesMaskImprSrcUnsafe(105))
			mM1 = RandStringBytesMaskImprSrcUnsafe(32)
			mM2 = RandStringBytesMaskImprSrcUnsafe(32)
			//mM3 = RandStringBytesMaskImprSrcUnsafe(32)
		} else {
			tSn = fmt.Sprintf("sn_%s", RandStringBytesMaskImprSrcUnsafe(51))
			mM1 = RandStringBytesMaskImprSrcUnsafe(16)
			mM2 = RandStringBytesMaskImprSrcUnsafe(16)
			//mM3 = RandStringBytesMaskImprSrcUnsafe(16)
		}
